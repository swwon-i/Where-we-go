...
| | |
|---|---|
| 인허가 적재 | 서울 일반·휴게음식점 **686,164행** → 유효 646,848 · 거절 39,316 (받기 59초 + 적재 3분 10초) |
| 일일 갱신 | 첫 자동 회차(2026-09-22): **신규 446 · 폐업 전환 527 · 소멸 0 · 좌표 이동 13** |
| 검수 | SQL 규칙 6종 — 좌표 결측 39,315 · 상호·주소 중복 31,876 · 상태·폐업일 불일치 31 · 좌표 이동 13 · 상호 결측 1 |
...
- **받기** — 공공데이터포털이 안내하는 LOCALDATA 파일 서버에서 서울 파일 두 개를 받는다(키 불필요, 이용허락 제한 없음). `.part` 로 받아 파일 이름·39컬럼 헤더·크기를 검사한 것만 제자리로 옮긴다 — 받다가 실패하면 기존 파일로 적재한다.
...
- **일일 갱신은 노트북의 예약 작업이다.** 절전(Modern Standby) 중에는 깨어나지 않아 22:00 로 옮겼다. 켜 두지 않은 날은 회차가 비고, 다음 실행이 그날의 최신 파일을 받아 이어진다. 원본은 2일 전 기준이다.
...
GitHub 한도(100MB)를 넘는다. 인허가 파일은 `etl.fetch` 가 받아 오고, 교통 데이터의 출처와 배치는 [etl/README.md](etl/README.md) 를 본다.

```bash
py -m venv .venv
./.venv/Scripts/python.exe -m pip install -r etl/requirements.txt
./.venv/Scripts/python.exe -X utf8 -m etl.fetch --source all       # 인허가 원본 받기 (약 1분, 209MB)
...
서버 한 대(Google Cloud · AWS · Oracle)에 올리는 방법은 [docs/deploy.md](docs/deploy.md) — HTTPS, DB 옮기기, 매일 22:00 갱신까지.
